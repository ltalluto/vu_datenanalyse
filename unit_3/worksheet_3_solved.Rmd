...
1. Formulate a set of hypotheses testing effects of productivity and lake size on spm. Choose variables wisely, then run a formal hypothesis test using MLR.
```{r}
...
lakes<-lakes[,-c(2,3,4)]
```
4. Identify the best **simple** linear regression model and a **full** model with only one predictor or all non-redundant ones to start a forward and a backward model building process.
```{r}
mlr.full<-lm(spm~.,data=lakes) # makes a model with all predictors available
summary(mlr.full)
...
5. Using the functions `add1`and `drop1` terms can be added to or removed from models. Follow a forward and a backward model building process until a final result and compare the two final models.
```{r}
...
```
6. Use `step` to identify a *good* model based on the AIC. How does this compare to the models created above? Check model assumptions and decide for a final model.
```{r}
