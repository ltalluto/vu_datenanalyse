...
May be useful in particular cases, examples:
$$
Flux=k \cdot \Delta C
$$
$$
...
$$
Fitting a model to a dataset here gives $R^2=1$ with two fractions partitioned for $\log k$ and $\log \Delta C$.
...
* start simple and add predictors
* do both, i.e. consider addition and removal of individual variables at any point to result in an 'optimized' model
...
A final plot of observed vs. predicted values allows to assess the predictive capacity of the model.
