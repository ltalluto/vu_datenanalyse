...
my_stats = data.frame(name = "yourname", mean = samp_mean, sd = samp_sd,
					  lower = samp_mean - 1.96*st_err, 
					  upper = samp_mean + 1.96*st_err)
...
2d. Use the `var` function to compute the variance of each group. Are the variances similar?

A rule of thumb: you may use equal variance tests if the larger variance is less than roughly 4x the smaller.

2e. Perform a t-test of the hypothesis you wrote in question 1. Use the R function `t.test`, and be sure to check the help with `?t.test` before you start. 

Pay attention to the options `alternative`, `paired`, and `var.equal`. How should you set them for this particular problem?
