...
### Grading
* Each half-day will be split between lecture and exercises.
* Your grade in the course will be based on completion of the exercises and submission of a short written protocol each day.


## Course Schedule
<div class="left lt">

![](../img/sched1.png){width=80%}
</div>

<div class="right rt">
![](../img/sched2.png){width=80%}
</div>
...
* (For the adventurous) Write your protocols using **Rmarkdown** (combines text, code, output, and graphics).
   - There will be an example during the exercises.
...
mat_and = matrix(c(F,F,F,T), nrow=2, dimnames=list(c("f", "t"), c("f", "t")))
kable(mat_and, caption = "Logical & (and)")
```

```{r echo = FALSE, results='asis'}
mat_and = matrix(c(F,T,T,T), nrow=2, dimnames=list(c("f", "t"), c("f", "t")))
kable(mat_and, caption = "Logical | (or)")
```
...
iris$Sepal.Length
iris[,1] ## [,1] means get every row in the first column
iris[,"Sepal.Length"] ## another way to access by name
```
```{r echo = FALSE, results = 'asis'}
kable_styling(kable(matrix(iris$Sepal.Length, nrow=10)), font_size = 12)
```
