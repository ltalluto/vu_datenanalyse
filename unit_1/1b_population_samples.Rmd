...
* R supports colors using the common HTML color coding: `#RRGGBB`
	...
	- Color pickers online help you translate a color in real life to a coded color
...
<div class="left lt">
* `ggplot2` is a **package**: packages extend the base functionality of R
* You can install packages with `install.packages()` (just once)
* You load packages with `library` (once per session)

### Using ggplot
* `ggplot` always works on data.frames, so make sure your data is in one
* Each plot defines **aesthetics** using `aes`
	- What to plot
	- In what position (x-axis, y-axis, etc)
	- How to visualise structure in data (e.g., color by categories or variables)
* You then add **geometries** to tell ggplot how to plot the data
* Here we create a plot, define a single variable plotted in the `x` position, and set a histogram geometry
* We can easily add other customisations (e.g., axis labels)

</div>

<div class="right rt">
```{r eval = FALSE}
# only do this once, if you do not have the package already installed
...
```{r, echo = FALSE}
boxplot(Sepal.Length ~ Species, data = iris, boxwex=0.4, notch = TRUE)
```
...
med = median(iris$Sepal.Length[iris$Species == "versicolor"])
text(2.15, med, "median", col=cols[3], pos=4)

lines(c(2.75, 2.75), c(6.35, 6.65), col=cols[4])
text(2.75, 6.5, "~95 % CI", col = cols[4], pos = 2)

text(3, 0.985*min(iris$Sepal.Length[iris$Species == "virginica"]), 
...
## Programming in R: tabular applies
